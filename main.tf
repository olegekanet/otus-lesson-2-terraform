...
resource "yandex_compute_instance" "vm_terraform_1" {
  depends_on = [
    local_file.cloud_config
  ]
  name        = "vm1"
  zone        = var.yandex_zone
  platform_id = "standard-v2" # Используем стандартный тип VM, можно изменить по необходимости
  resources {
    cores  = 2
    memory = 2
  }
  boot_disk {
    initialize_params {
      image_id = var.image_id # ID образа операционной системы
    }
  }

  network_interface {
    subnet_id = yandex_vpc_subnet.lab_subnet.id
    # Привязка публичного IP-адреса к виртуальной машине
    nat = true
  }

  # metadata = {
  #   ssh-keys = "ubuntu:${local_file.ssh_key_pub.content}"
  # }
  metadata = {
    user-data = "${file("./cloud-config.yaml")}"
  }

  # Опциональные теги для управления ресурсами
  labels = {
...
resource "local_file" "hosts_file" {
  ...
  content  = <<-EOT
    [servers]
    ${yandex_compute_instance.vm_terraform_1.network_interface[0].nat_ip_address} ansible_ssh_user=ubuntu ansible_ssh_private_key_file="${path.module}/id_rsa"
  EOT
}
...
resource "null_resource" "ansible_provisioner" {
  ...
  depends_on = [
    yandex_compute_instance.vm_terraform_1
  ]
  ...
  provisioner "local-exec" {
    command = "ANSIBLE_HOST_KEY_CHECKING=false ansible-playbook -i '${path.module}/hosts' ${path.module}/playbook.yaml"
  }
}
